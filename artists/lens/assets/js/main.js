...
var main = (function($) {
	...
	loadData: function(userid, offset){
		$.ajax({
	    ...
	    success: function(json) {
				...
				$("#affection").append(json.stats.user_favourites);
				$("#picture_count").append(json.stats.user_deviations);
				$("#userphoto").attr('src',json.profile_pic);
				$("#bio").append(json.bio)
				main.loadPictures(userid, offset);
	    }
	  });
	},
	loadPictures: function(userid,offset){
	  $.ajax({
	    ...
	    success: function(json) {
	        ...
	        $("#thumbnails").append("<article><a class=\"thumbnail\" href=\"" +this.image_url +"\"DAurl=\""+this.url + "\"><img src=\""+this.thumb_url+"\" alt=\""+this.title+"\" /></a><h2>"+this.title+"</h2></article>");
	      });
